...
class Autocomplete extends Component {
  ...
  onClick = e => {
    ...
    let suggestionSelected = filteredSuggestions.filter(suggestion => {
      return suggestion.id_livro === Number(e.currentTarget.id) ? suggestion : null;
    });
        ...
        suggestionsListComponent = (
          <ul className="suggestions">
            {filteredSuggestions.map((suggestion, index) => {
              ...
              return (
                <li className={className} key={index} onClick={onClick} id={suggestion.id_livro}>
                  <span style={{ minWidth: "25px", display: "inline-block" }}>{suggestion.id_livro}</span> - <span>{suggestion[field]}</span>
                </li>
              );
            })}
          </ul>
